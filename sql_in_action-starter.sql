...
SELECT * from intro_sql.final_airbnb where price order by price asc limit 1;
SELECT * from intro_sql.final_airbnb where price order by price desc limit 1;
-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 5 >>>>>>>>>>>>>>>>>>>>>>>
...
SELECT host_name, count(*) as number_of_listings
from intro_sql.final_airbnb
group  by host_name
order by count(*) desc
limit 1;
...
SELECT (SELECT count(*) as number_above from intro_sql.final_airbnb where price < (SELECT avg(price) from intro_sql.final_airbnb)) / (select count(*)  as total_listings
							from intro_sql.final_airbnb) * 100 as percent_above,
    (SELECT count(*) as number_below from intro_sql.final_airbnb where price > (SELECT avg(price) from intro_sql.final_airbnb)) / (select count(*)  as total_listings
							from intro_sql.final_airbnb) * 100 as percent_below
/* Result: percent_above = 65 and percent_below = 35 */